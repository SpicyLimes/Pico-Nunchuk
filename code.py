...
# Accelerometer settings for tilt (orbit mode)
ACCEL_CENTER_X = 512  # Neutral X accelerometer value (10-bit: 0-1023)
ACCEL_CENTER_Y = 512  # Neutral Y accelerometer value
ACCEL_DEADZONE = 50   # Ignore small tilts
ACCEL_SENSITIVITY = 10  # Mouse pixels per tick at max tilt

# Button tap detection
TAP_MAX_DURATION = 0.3  # Max seconds for a press to count as a tap
TAP_MOVE_THRESHOLD = 10  # Max joystick/accel movement during tap
...
def accel_active(ax, ay):
    """Check if accelerometer tilt is outside deadzone."""
    return abs(ax - ACCEL_CENTER_X) > ACCEL_DEADZONE or abs(ay - ACCEL_CENTER_Y) > ACCEL_DEADZONE
...
def main():
    ...
    while True:
        ...
        accel = nunchuk.acceleration

        jx, jy = joy[0], joy[1]  # 0-255
        ax, ay, az = accel[0], accel[1], accel[2]  # 10-bit
        c_pressed = buttons.C
        z_pressed = buttons.Z

        # Detect movement for tap detection
        joy_moved = joy_active(jx, jy)
        accel_moved = accel_active(ax, ay)

        # Update button states
        c_event = btn_c.update(c_pressed, now, joy_moved)
        z_event = btn_z.update(z_pressed, now, accel_moved)
        ...
        if btn_c.is_pressed and btn_c.was_held:
            ...
            # Z HELD: Orbit via tilt (right mouse button + drag)
            mx = scale_axis(ax, ACCEL_CENTER_X, ACCEL_DEADZONE, ACCEL_SENSITIVITY)
            my = -scale_axis(ay, ACCEL_CENTER_Y, ACCEL_DEADZONE, ACCEL_SENSITIVITY)
